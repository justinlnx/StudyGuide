...
public class Grid {
    ...
    public void move(Direction direction) {
        for (int i = 0; i < SIZE; i++) {
            ...
            //main tile group algorithm
            if (!(isEmptyTileSet(tileSet))) slide(tileSet);
        }
    }

    private boolean isEmptyTileSet(List<Tile> tileSet) {
        for (Tile tile: tileSet) {
    ...
    private void slideToEdge(List<Tile> tileSet) {
        for (int i = 0; i < tileSet.size(); i++) {
            if (remainingIsZero(tileSet, i)) return;
    ...
    private boolean remainingIsZero(List<Tile> tileSet, int i) {
        ...
        return (isEmptyTileSet(remainingTile));
    }
